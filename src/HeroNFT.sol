...
contract HeroNFT is ERC721, Ownable {
    ...
    function mintHero(address to, string calldata name, uint8 heroClass) external onlyGame returns (uint256) {
        require(heroClass <= 2, "Invalid class");

        uint256 tokenId = _tokenIdCounter++;

        // 用区块信息生成伪随机数（测试用，生产用VRF）
        uint256 rand = uint256(keccak256(abi.encodePacked(block.timestamp, block.prevrandao, to, tokenId)));
